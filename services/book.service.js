    ...
    return storageService.query(BOOK_KEY)
        .then(books => {
            if (filterBy.title) {
                ...
                books = books.filter(book => titleRegex.test(book.title))
            }
            if (filterBy.price) {
                books = books.filter(book => book.listPrice.amount >= filterBy.price)
            }
            if (filterBy.author) {
                const authorRegex = new RegExp(filterBy.author, 'i')
                books = books.filter(book => authorRegex.test(book.authors))
            }

            if (filterBy.category) {
                const categoryRegex = new RegExp(filterBy.category, 'i')
                books = books.filter(book => categoryRegex.test(book.categories))
            }
...
function getDefaultFilter() {
    return { title: '', price: 0, author: '', category: '' }
}
...
//         books = []
//         books.push(_createBook('Learning React', 50))
//         books.push(_createBook('Habits', 90))
//         books.push(_createBook('Grit', 100))
